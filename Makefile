...
.PHONY: install
## `install`: Fetch dependencies needed to run `go-web-common`
install:
...
.PHONY: run
## :
## `run`: Run `go-web-common` in production mode
...
.PHONY: run-debug
## `run-debug`: Run `go-web-common` in debug mode
...
.PHONY: docker-gen
## :
## `docker-gen`: Create a production docker image for `go-web-common`
docker-gen:
...
.PHONY: docker-debug
## `docker-debug`: Create debug-friendly docker images for `go-web-common`
docker-debug:
	echo "Building docker image \`$(IMAGE):$(VERSION)\`..."
